...
app.post('/api/auth/register', async (req, res) => {
  ...
  try {
    ...
    // Default role is 'buyer' and discount is 10% for new users (CofC community)
    const userId = generateId('usr_');
    const defaultRole = 'buyer';
    const defaultDiscount = 0.10; // 10% discount for buyers

    // 3. Insert new user into the database
    const insertQuery = `
      INSERT INTO users (id, name, username, password_hash, role, email, discount)
      VALUES ($1, $2, $3, $4, $5, $6, $7)
      RETURNING id, name, username, role, email, discount
    `;
    const result = await pool.query(insertQuery, [userId, name, username, passwordHash, defaultRole, email, defaultDiscount]);
