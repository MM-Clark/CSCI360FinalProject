  ...
  const Login = () => {
    ...
    const RegistrationForm = () => {
        ...
        const handleRegister = async () => {
            ...
            if (!regName || !regUsername || !regPassword || !regEmail) {
            ...
            setRegLoading(true);
            try {
                const data = await api.register(regName, regUsername, regPassword, regEmail);
                ...
                // Error structure usually contains response.statusText, which is caught by api.request
                setRegError(err.message || 'Registration failed. Please check if username/email already exists.');
        ...
        return (
            <div className="space-y-4">
                ...
                <input type="text" value={regUsername} onChange={e => setRegUsername(e.target.value)} placeholder="Username" className="w-full p-3 border border-gray-700 rounded-lg bg-gray-700 text-white" />
    ...
    const LoginForm = () => (
        <div className="space-y-4">
            ...
            {/* Removed userType selector to keep focus on login/signup flow */}
            <input 
              type="text" 
              value={username} 
