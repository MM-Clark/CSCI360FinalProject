...
// Configuration is loaded from window global variables (set in index.html)
// Ensure you have replaced the placeholder values in index.html with your actual Firebase keys.
...
function App() {
  ...
  const Login = () => {
    ...
    const handleLogin = async () => {
      ...
      try {
        setLoading(true);
        // This will trigger the onAuthStateChanged listener to fetch the profile and set currentUser
    ...
    const LoginForm = () => (
        <div className="space-y-4">
            ...
            <input 
              type="email" 
              value={email} 
              onChange={(e) => {setEmail(e.target.value); setLoginError(false);}} // Fixed binding
              placeholder="Email" 
            ...
            <input 
              type="password" 
              value={password} 
              onChange={(e) => {setPassword(e.target.value); setLoginError(false);}} // Fixed binding
